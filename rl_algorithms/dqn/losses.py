...
class R2D1Loss:
    def __call__(
        ...
        actions = actions.narrow(-1, 0, actions.shape[1] - 1).long().unsqueeze(-1)
        rewards = rewards.narrow(1, 0, rewards.shape[1] - 1).unsqueeze(-1)
        dones = dones.narrow(1, 0, dones.shape[1] - 1).unsqueeze(-1)
